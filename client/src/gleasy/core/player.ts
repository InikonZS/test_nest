...
import { ColliderList } from "./collider";
...
export class Player{
  ...
  procMoves(world: ColliderList, deltaTime: number, /*obj: { reactLine: any; }*/){
    if (!world){
        return;
    }
  ...
  getMatrix(aspect: number){
      return makeCameraMatrix(aspect, this.camRX, this.camRY, this.posX, this.posY, this.posZ);
  }
}
